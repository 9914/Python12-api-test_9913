...
# @File     : A_12_ByLoandId_test_case.py
# @Software : PyCharm
# @Explain   : 标的投资记录接口测试用例
...
#充值
excel_date_geneInvestsByLoanId = excel_date.get_case('geneInvestsByLoanId')
...
class HttpCase(unittest.TestCase):
    ...
    # 加 * 号，去掉一层外套
    @data(*excel_date_geneInvestsByLoanId)
    #@unpack
    def test_http_case(self,i):
        ...
        try:
            ...
            if json.loads(res.get_text())['code'] == '10001':  # 查询成功，进行数据库校验
                ...
                try:
                    if recroed is None:
                        my_log.info("数据库校验-->标还没有人来投资")
                    elif recroed['Id'] == mark_record['Id']:
                        my_log.info("数据库校验-->查询成功，最新记录ID：{}".format(recroed['Id'] ))
                    elif mark_record is None and recroed is not None:
            ...
            excel_date.write_by_case_id(sheet_name='geneInvestsByLoanId', case_id=i.case_id,
